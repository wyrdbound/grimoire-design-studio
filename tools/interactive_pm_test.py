...
def create_project_interactive(pm: ProjectManager):
    ...
    project_path = input("📁 Project directory path (or press Enter for current dir): ").strip()
    if not project_path:
...
def load_system_interactive(pm: ProjectManager):
    ...
    try:
        ...
        for name, component_dict in components:
            ...
            icon = "📋" if name == "Models" else "🔄" if name == "Flows" else "📚" if name == "Compendiums" else "🎯" if name == "Tables" else "📄" if name == "Sources" else "💭"
